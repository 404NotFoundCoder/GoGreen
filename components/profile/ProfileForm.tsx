...
export function ProfileForm() {
  const { nickname, loading, error, save } = useProfile();
    ...
    <div className="rounded-2xl border-[0.5px] border-[var(--color-muted)] bg-[var(--color-surface)] p-4">
      ...
      {!editing ? (
        <div className="mt-4 flex min-h-[44px] items-center gap-2">
          <p className="min-w-0 flex-1 truncate text-base font-medium text-[var(--color-ink)]">
            {nickname || "—"}
          </p>
          <button
            type="button"
            onClick={startEdit}
            className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[var(--color-muted)] bg-[var(--color-white)] text-[var(--color-ink-secondary)] transition hover:border-[var(--color-primary-strong)]/40 hover:bg-[var(--color-primary-light)]/50 hover:text-[var(--color-ink)]"
            aria-label="編輯暱稱"
          >
        ...
        <>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button
              ...
              disabled={busy}
              onClick={() => void onSave()}
              className="min-h-[44px] flex-1 rounded-full bg-[var(--color-primary-strong)] px-4 py-2.5 text-sm font-medium text-[var(--color-white)] disabled:opacity-50 sm:flex-none sm:min-w-[120px]"
            >
              {busy ? "儲存中…" : "儲存"}
            </button>
            <button
              ...
              disabled={busy}
              onClick={cancelEdit}
              className="min-h-[44px] flex-1 rounded-full border-[0.5px] border-[var(--color-muted)] bg-[var(--color-bg)] px-4 py-2.5 text-sm font-medium text-[var(--color-ink)] disabled:opacity-50 sm:flex-none"
            >
              取消
            </button>
          </div>
